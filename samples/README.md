...
versionnés (voir `.gitignore`) : ce sont tes sons, ton terrain, ton micro.

## Convention

    samples/
      reference/    des sons à DÉTECTER (le côté positif)
      negative/     le fond sonore du terrain, SANS source (le côté négatif)

Le dossier `negative/` est le plus important et le plus souvent oublié : un
seuil ne se règle pas sur les positifs seuls. Sans lui, on sait qu'un événement
...
docker compose run --rm --no-deps \
  ...
  capture python -m app.tools.selftest /ref/reference.wav --segment 3
