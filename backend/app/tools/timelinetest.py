...
mérite un test :
  ...
  · une fenêtre où `Dog` marque 0,40 et `Speech` 0,46 s'appelle `Speech` — le
    chien doit rester visible dans `dog_frames`, sinon il disparaît ;
...
from ..analysis.timeline import HOP_S, WINDOW_S, build_timeline, dog_scores
from ..classifier.yamnet_litert import DOG_CLASS_NAMES
...
def test_pur() -> None:
    ...
    m = np.array(
        [
            ...
            [0.10, 0.40, 0.46],  # argmax Speech, mais Dog à 0.40 : le chien doit rester vu
            [0.05, 0.05, 0.02],  # sous le seuil : troue la timeline
            [0.00, 0.00, 0.00],
        ],
        dtype=np.float32,
    )
    r = build_timeline(m, noms, n_samples=5 * 15600, min_score=0.3, dog_threshold=0.35)
    ...
    check("le chien est vu MALGRÉ l'argmax", len(r["dog_frames"]) == 1,
          f"{len(r['dog_frames'])} fenêtre(s)")
    if r["dog_frames"]:
        df = r["dog_frames"][0]
        check("et l'écart est nommé, pas caché",
              df["sound"] == "Speech" and df["dog"] == 0.4,
              f"canin {df['dog']} vs {df['sound']} {df['sound_score']}")

    check("dog_max vient du GROUPE, pas de la timeline", r["dog_max"] == 0.4, str(r["dog_max"]))
    ...
    # Le groupe canin se résout par NOM : une matrice où aucun nom ne
    # correspond doit rendre des zéros, pas lever ni prendre la colonne 0.
    zeros = dog_scores(m, ["Rien", "Ailleurs", "Aucun"])
    check("groupe canin introuvable → zéros, sans lever",
...
def test_reel(chemin: str) -> None:
    ...
    r = build_timeline(
        ...
        dog_threshold=settings.dog_threshold,
    )
    ...
    print(f"  canin max {r['dog_max']} — seuil {r['dog_threshold']}, "
          f"{len(r['dog_frames'])} fenêtre(s) retenue(s)")
    for s in r["timeline"]:
        print(f"    {s['interval']:<24} {s['sound']:<28} {s['score']}")
    if r["dog_best"]:
        b = r["dog_best"]
        print(f"  meilleur moment canin : {b['interval']} → {b['dog']} "
    ...
    check("le canin atteint bien le seuil ou pas, mais est calculé",
          r["dog_max"] >= 0.0 and (r["dog_frames"] == []) == (r["dog_max"] < r["dog_threshold"]),
          f"max {r['dog_max']} / {len(r['dog_frames'])} fenêtre(s)")
...
def main() -> int:
    ...
    print(f"  groupe canin : {list(DOG_CLASS_NAMES)}")
    test_pur()
