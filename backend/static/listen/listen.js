...
function _carte(projet, actif) {
  ...
  // Donner un extrait à un projet EXISTANT. Sans ce bouton, enrichir un groupe
  // demandait d'en créer un second pour la même chose — et l'aide de la modale
  // renvoyait à un « + Réf » qui vit ici, mais pour autre chose (marquer un
  // segment comme référence dans le catalogue).
...
async function renommeProjet(projet) {
  ...
  if (nom === null) return;
  if (!nom.trim()) { _projetNote('Le nom ne peut pas être vide.', 'avert'); return; }
  try {
    ...
    _projetNote('Projet renommé.', 'ok');
    await rendProjet();
  } catch (err) {
    _projetNote('Renommage refusé : ' + err.message, 'avert');
  }
}
...
async function supprimeProjet(projet) {
  ...
  try {
    ...
    // l'actif, sinon les tableaux resteraient vides sans qu'on sache pourquoi.
    if (projetVue === projet.id) memoriseProjetVue(null);
    _projetNote(r.message, 'ok');
    await rendProjet();
  } catch (err) {
    _projetNote('Suppression refusée : ' + err.message, 'avert');
  }
}
...
async function rendProjet() {
  ...
  enTete.textContent =
    ...
    + 'surveiller aussi — le poste de terrain est prévenu et redémarre sa '
    + 'capture audio dans les 15 secondes.';
...
function rendNouveau() {
  ...
  aide.textContent =
    ...
    + 'son cherché. Un projet sans extrait fonctionne aussi : tu pourras lui en '
    + 'donner un plus tard par « Référence… », sur sa carte.';
...
async function versWav(file) {
  ...
  // Déjà du WAV : on l'envoie tel quel, le serveur le lit.
  ...
  // décodeur MP3, par choix délibéré (350 Mo de ffmpeg évités) — et cette
  // décision ne se revoit pas depuis une modale. Le navigateur, lui, décode
  // nativement.
  ...
  try {
    ...
    // La fréquence native suffit : le serveur rééchantillonne à 16 kHz.
...
async function traiteExtrait(fichier) {
  ...
  let wav;
  try {
    wav = await versWav(fichier);
  } catch (err) {
    zone.textContent = '';
    _projetNote('Fichier illisible : ' + err.message, 'avert');
    return;
  }
  ...
  try {
    ...
    _projetNote('Extrait de ' + data.duree_s + ' s analysé. Décoche les classes '
      + 'qui ne conviennent pas.', 'ok');
  } catch (err) {
    zone.textContent = '';
    _projetNote('Analyse impossible : ' + err.message, 'avert');
  }
}
...
// `actuelles` : le groupe DÉJÀ surveillé, quand on enrichit un projet existant.
// Absent à la création — il n'y a alors rien à préserver, et tout est coché.
function afficheClasses(items, actuelles, zoneCible) {
  ...
  // jamais les seules détectées : un extrait où une classe surveillée ne réagit
  // pas la ferait disparaître de la liste, donc du projet à l'enregistrement.
  // Une régression décidée par un extrait, en silence.
  ...
  for (const nom of noms) {
    ...
    const cb = document.createElement('input');
    cb.type = 'checkbox';
    // La VALEUR est le nom du modèle : c'est lui qui part en base, et c'est lui
    // que YAMNet résoudra au chargement. Le français n'est qu'un affichage.
    cb.value = c.nom;
    ...
    lab.appendChild(s);
    if (c.fr && c.fr !== c.nom) {
      // Le français À CÔTÉ du nom anglais : on ne peut pas demander de décocher
    ...
    // D'où vient la ligne. Sans ce mot, une classe décochée ne se distingue pas
    // d'une classe oubliée, et l'opérateur ne sait pas ce qu'il regarde.
    if (actuelles) {
...
// les classes actuelles restent cochées, celles que l'extrait apporte arrivent
// décochées.
//
...
async function extraitPourProjet(projet, boite, fichier) {
  ...
  catch (err) { _projetNote('Fichier illisible : ' + err.message, 'avert'); return; }
...
async function appliqueClasses(projet, zone, bouton) {
  ...
  if (!classes.length) {
    _projetNote('Un projet sans aucune classe ne surveillerait rien : garde au '
  ...
  try {
    ...
    _projetNote('Modification refusée : ' + err.message, 'avert');
...
async function creeProjet(nom) {
  ...
  if (!classes.length) {
    _projetNote('Un projet sans aucune classe ne surveillerait rien : '
      + 'propose d\'abord des classes, ou donne-lui un extrait par '
      + '« Référence… » après création.', 'avert');
    return;
  }
...
function voirProjet(projet) {
  ...
  $('projet').close();
  // ⚠️ Le TITRE doit suivre. Il ne se mettait à jour qu'au chargement de la
  // page : après un « Voir », il annonçait encore l'ancien projet — donc
  // l'en-tête mentait sur ce que les tableaux montraient.
  majNomProjet();
  chargeSamples();
  chargeCaptures();
  chargeQC();
...
async function majNomProjet() {
  ...
  try {
    ...
    if (surveilleEl) {
      surveilleEl.textContent = d.actif
        ? d.actif.nom
